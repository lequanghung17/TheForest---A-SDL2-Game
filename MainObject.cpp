...
void MainObject::HandelInputAction(SDL_Event events, SDL_Renderer* screen)
{
	...
	if (events.type == SDL_MOUSEBUTTONDOWN)
	{
		if (events.button.button == SDL_BUTTON_LEFT)
		{
			...
			p_bullet->LoadImg("img//52.png", screen);
...
{
	map_data.start_x_ = x_pos_ - (SCREEN_WIDTH / 2); // khi di chuyen duoc 1/2 Map thi Map moi se xuat hien
	if (map_data.start_x_ < 0)
	{
		map_data.start_x_ = 0;
	}
	else if (map_data.start_x_ + SCREEN_WIDTH >= map_data.max_x_) // dung lai khi da het chieu dai Map
	{
